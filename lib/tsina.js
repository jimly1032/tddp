...
exports.tapi = {
	 ...
	 */
	 oauth: function(req,res,option){
		...
		this.getAccessToken(req,res,option,function(){
			...
			that.users_show(req,res,req.session.uid,function(data){
	...
	logout: function(req,res,callback){
		urllib.request(this.baseurl+'account/end_session.json',this.parma,function(err,data,res){
			console.log('is ready to lougout');
